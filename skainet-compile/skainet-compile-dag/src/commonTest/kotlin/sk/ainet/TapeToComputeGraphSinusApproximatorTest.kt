...
class TapeToComputeGraphSinusApproximatorTest {
    ...
    fun testTapeToComputeGraphWithSinusApproximator() {
        ...
        println(model.describe(Shape(1, 1), ctx, FP32::class))
        ...
        /*
        tape.recordOperation(inputOp, inputs = emptyList(), outputs = listOf(spec("input_out", listOf(1, 1))))
        tape.addBySpec(
            dense1Op,
            inputs = listOf(spec("dense1_in", listOf(1, 1))),
            outputs = listOf(spec("dense1_out", listOf(1, 16)))
        )
        tape.addBySpec(
            relu1Op,
            inputs = listOf(spec("relu1_in", listOf(1, 16))),
            outputs = listOf(spec("relu1_out", listOf(1, 16)))
        )
        tape.addBySpec(
            dense2Op,
            inputs = listOf(spec("dense2_in", listOf(1, 16))),
            outputs = listOf(spec("dense2_out", listOf(1, 16)))
        )
        tape.addBySpec(
            relu2Op,
            inputs = listOf(spec("relu2_in", listOf(1, 16))),
            outputs = listOf(spec("relu2_out", listOf(1, 16)))
        )
        tape.addBySpec(
            outOp,
            inputs = listOf(spec("output_in", listOf(1, 16))),
            outputs = listOf(spec("output_out", listOf(1, 1)))
        )
