...
class DoubleTensorsTest {
    ...
    fun testTensorDSLSyntaxWithDoubles() {
        ...
        var captured: WeightAndBias<Double>? = null
        val result = runCatching {
            data {
                captured = WeightAndBias(
                    "layer1WandB",
                    tensor<FP32, Double> {
                        shape(1, 16) {
                            init { indices ->
        ...
        result.exceptionOrNull()?.let { throwable ->
            assertTrue(
                ...
                "Expected ClassCastException but got ${throwable::class.simpleName}"
            )
        } ?: run {
            val wandB = requireNotNull(captured)
            assertEquals(layer1Weigths.first(), wandB.weights.data[0, 0])
            assertEquals(layer1Bias.last(), wandB.bias!!.data[15])
        }
    }
}
